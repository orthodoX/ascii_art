defmodule AsciiArt.Matrix do
  ...
  ## Example
      ...
      %{0 => %{0 => "x", 1 => "o", 2 => "x"}}
  """
  def from_list(list) when is_list(list), do: do_from_list(list)

  defp do_from_list(list, map \\ %{}, index \\ 0)
  defp do_from_list([], map, _index), do: map

  defp do_from_list([head | tail], map, index) do
    map = Map.put(map, index, do_from_list(head))

    do_from_list(tail, map, index + 1)
  end
  ...
  ## Example
      ...
      [["x", "o", "x"]]
  """
  def to_list(matrix) when is_map(matrix), do: do_to_list(matrix)
